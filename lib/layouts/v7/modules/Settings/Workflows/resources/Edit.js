...
Settings_Vtiger_Edit_Js("Settings_Workflows_Edit_Js", {
    ...
    registerFormSubmitEvent : function() {
        ...
        var params = {
            submitHandler: function(form) {
                ...
                if (jQuery('[name="workflow_trigger"]').val() == '6' && jQuery('#schtypeid').val() == '9') {
                    var minuteInterval = parseInt(jQuery('#schminuteinterval').val(), 10);
                    if (isNaN(minuteInterval) || minuteInterval < 1) {
                        app.helper.showErrorNotification({'message': app.vtranslate('LBL_MINUTE_INTERVAL_REQUIRED')});
                        return false;
                    }
                }
   ...
   loadFieldSpecificUi: function (fieldSelect) {
      ...
      if (fieldInfo.type == 'owner') {
         var ownerSelect = fieldSpecificUi.is('select') ? fieldSpecificUi : fieldSpecificUi.find('select');
         if (ownerSelect.length && ownerSelect.find('option[value="$source_assigned_user_id"]').length == 0) {
            var sourceOwnerOption = jQuery('<option></option>').val('$source_assigned_user_id').text('» Assegnatario del record sorgente');
            if (fieldInfo.value == '$source_assigned_user_id') {
               sourceOwnerOption.attr('selected', 'selected');
            }
            ownerSelect.prepend(sourceOwnerOption);
         }
      }
   ...
   registerCcAndBccEvents: function () {
	...
	registerEventForChangeInScheduledType : function() {
		...
		jQuery('#schtypeid').on('change', function(e){
			...
			thisInstance.hideScheduledMinuteInterval();

			if(value == '1') {	//hourly
				thisInstance.hideScheduledTime();
			} else if (value == '8') { // every minute
				thisInstance.hideScheduledTime();
			} else if (value == '9') { // every N minutes
				thisInstance.hideScheduledTime();
				thisInstance.showScheduledMinuteInterval();
	...
	hideScheduledMinuteInterval : function() {
		jQuery('#scheduledMinuteInterval').addClass('hide');
	},

	showScheduledMinuteInterval : function() {
		jQuery('#scheduledMinuteInterval').removeClass('hide');
	},
