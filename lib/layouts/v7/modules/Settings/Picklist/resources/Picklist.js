...
var Settings_Picklist_Js = {
    ...
    getHashParams : function() {
        ...
        jQuery.each(hash.split('&'), function(index, pair) {
            if (!pair.length) {
                return;
            }
            var pairParts = pair.split('=');
            var key = decodeURIComponent(pairParts[0] || '');
            var value = decodeURIComponent(pairParts.slice(1).join('=') || '');
            if (key.length) {
                params[key] = value;
            }
        });
    ...
    updateSelectionHash : function() {
        ...
        var selectedField = jQuery('#modulePickList option:selected');
        if (!selectedModule || !selectedField.length) {
            return;
        }
        var hashParts = [
            'source_module=' + encodeURIComponent(selectedModule),
            'fieldid=' + encodeURIComponent(selectedField.val())
        ];
        var blockId = selectedField.data('block');
        if (blockId) {
            hashParts.push('block=' + encodeURIComponent(blockId));
        }
        window.location.hash = hashParts.join('&');
    },

    applyHashSelection : function() {
        ...
        this.pendingHashSelection = hashParams;
        var selectedModule = hashParams.source_module;
        if (selectedModule && jQuery('#pickListModules').val() !== selectedModule) {
            jQuery('#pickListModules').val(selectedModule).trigger('change');
            return;
        }

        if (hashParams.fieldid && jQuery('#modulePickList').length) {
            jQuery('#modulePickList').val(hashParams.fieldid);
        }
        if (jQuery('#modulePickList').length) {
	...
	registerModuleChangeEvent : function() {
		jQuery('#pickListModules').on('change',function(e){
			...
			app.request.post({data: params}).then(function(error, data){
                ...
                var pendingHashSelection = Settings_Picklist_Js.pendingHashSelection;
                if (pendingHashSelection && pendingHashSelection.fieldid && jQuery('#modulePickList option[value="' + pendingHashSelection.fieldid + '"]').length) {
                    jQuery('#modulePickList').val(pendingHashSelection.fieldid);
                }
				jQuery('#modulePickList').trigger('change');
			});
		});
	...
	registerModulePickListChangeEvent : function() {
		jQuery('#modulePickList').on('change',function(e){
            Settings_Picklist_Js.updateSelectionHash();
			var params = {
				module : app.getModuleName(),
				parent : app.getParentModuleName(),
			...
			app.request.post({data: params}).then(function(error, data){
                ...
                Settings_Picklist_Js.pendingHashSelection = null;
				app.helper.hideProgress();
			})
		})
	},
    ...
    registerSyncConfigEvent : function() {
        var container = this.getContainer();
        var syncCandidates = {};
        if (jQuery('#picklistSyncCandidates').length) {
            syncCandidates = JSON.parse(jQuery('#picklistSyncCandidates').val());
        }

        var populateSyncFields = function(selectedModule, selectedFieldId) {
            var fieldSelect = jQuery('#picklistSyncField');
            fieldSelect.html('<option value="">'+app.vtranslate('JS_SELECT_OPTION')+'</option>');
            if (!selectedModule || !syncCandidates[selectedModule]) {
                fieldSelect.trigger('change');
                return;
            }

            jQuery.each(syncCandidates[selectedModule], function(index, field) {
                var option = jQuery('<option></option>')
                    .val(field.fieldid)
                    .attr('data-module-name', field.module)
                    .text(app.vtranslate(field.fieldlabel, field.module));
                if (selectedFieldId && String(selectedFieldId) === String(field.fieldid)) {
        ...
        container.off('change', '#picklistSyncModule');
        container.on('change', '#picklistSyncModule', function(e) {
            populateSyncFields(jQuery(e.currentTarget).val());
        });

        container.off('click', '#addPicklistSyncField');
        container.on('click', '#addPicklistSyncField', function(e) {
            e.preventDefault();
            var moduleName = jQuery('#picklistSyncModule').val();
            var fieldOption = jQuery('#picklistSyncField option:selected');
            var fieldId = fieldOption.val();
            if (!moduleName || !fieldId) {
                app.helper.showErrorNotification({message: app.vtranslate('JS_PLEASE_SELECT_MODULE')});
                return;
            }

            if (jQuery('#picklistSyncList .picklistSyncRow[data-field-id="'+fieldId+'"]').length) {
                return;
            }

            var row = '' +
                '<tr class="picklistSyncRow" data-field-id="'+fieldId+'" data-module-name="'+moduleName+'" data-field-label="'+fieldOption.text()+'">' +
                    '<td>'+app.vtranslate(moduleName, moduleName)+' / '+fieldOption.text()+'</td>' +
                    '<td class="text-center"><a href="javascript:void(0)" class="removePicklistSyncField" title="'+app.vtranslate('LBL_DELETE_VALUE', app.getModuleName())+'"><i class="fa fa-trash-o"></i></a></td>' +
                '</tr>';
            jQuery('#picklistSyncList tbody').append(row);
            jQuery('#picklistSyncList .picklistSyncEmptyRow').addClass('hide');
        });

        container.off('click', '.removePicklistSyncField');
        container.on('click', '.removePicklistSyncField', function(e) {
            e.preventDefault();
            jQuery(e.currentTarget).closest('.picklistSyncRow').remove();
            if (!jQuery('#picklistSyncList .picklistSyncRow').length) {
                jQuery('#picklistSyncList .picklistSyncEmptyRow').removeClass('hide');
            }
        });

        container.off('click', '#savePicklistSync');
        container.on('click', '#savePicklistSync', function(e) {
            e.preventDefault();
            var linkedFieldIds = [];
            jQuery('#picklistSyncList .picklistSyncRow').each(function() {
                linkedFieldIds.push(jQuery(this).data('field-id'));
            });
            var params = {
                module : app.getModuleName(),
                parent : app.getParentModuleName(),
                action : 'SaveAjax',
                mode : 'saveSync',
                pickListFieldId : jQuery('#picklistSyncSourceFieldId').val(),
            ...
            app.request.post({data: params}).then(function(error, data) {
                app.helper.hideProgress();
                if(data) {
                    app.helper.showSuccessNotification({message: app.vtranslate('JS_PICKLIST_SYNC_SAVED_SUCCESSFULLY')});
                }
            });
        });

        if (jQuery('#picklistSyncModule').length) {
            vtUtils.showSelect2ElementView(jQuery('#picklistSyncModule'));
        }
        if (jQuery('#picklistSyncField').length) {
            vtUtils.showSelect2ElementView(jQuery('#picklistSyncField'));
        }

        populateSyncFields(jQuery('#picklistSyncModule').val());
    },
	...
	showDeleteItemForm : function(params) {
		...
		var callBackFunction = function(data) {
			...
			var deleteSelect = jQuery('[name="delete_value[]"]');
			deleteSelect.attr('data-skip-picklist-header-block', 'true');
			// The global modal initializer already called preparePicklistHeaders on this select
			// (before this callback ran), adding disabled=true to header options.
			// Remove it now so select2 initializes with them as selectable.
			deleteSelect.find('option[data-picklist-header]').prop('disabled', false);
			vtUtils.showSelect2ElementView(deleteSelect, {maximumSelectionSize: maximumSelectionSize, dropdownCss : {'z-index' : 100001}});
	...
	registerItemActions : function() {
        ...
        Settings_Picklist_Js.registerSyncConfigEvent();
	},
	...
	registerEvents : function() {
        ...
        Settings_Picklist_Js.applyHashSelection();
	}
}
