...
Vtiger_Detail_Js("Project_Detail_Js",{
	...
	*/
    ...
    milestoneFirst: function(a, b) {
        var aIsMilestone = a.type === 'milestone' ? -1 : 1;
        var bIsMilestone = b.type === 'milestone' ? -1 : 1;
        ...
        arr = arr.sort(function(a, b) {
            ...
            if (asc) {
                if (a[prop] === parseInt(a[prop], 10) && b[prop] === parseInt(b[prop], 10)) {
                    ...
                    result = new Date(a[prop]).getTime() - new Date(b[prop]).getTime();
                } else {
                    result = thisInstance.sortAlphabetically(a[prop], b[prop]);
                }
            } else {
                if (a[prop] === parseInt(a[prop], 10) && b[prop] === parseInt(b[prop], 10)) {
                    result = b[prop]-a[prop];
                } else if( thisInstance.isDate(a[prop]) && thisInstance.isDate(b[prop])) {
                    result = new Date(b[prop]).getTime() - new Date(a[prop]).getTime();
                } else {
                    result = thisInstance.sortAlphabetically(b[prop], a[prop]);
                }
            }
            return result !== 0 ? result : thisInstance.milestoneFirst(a, b);
        });
	...
	registerEvents : function(){
        ...
        // Construction flavor: when returning from a SalesOrder save, reopen the "Materiali" tab.
        if (decodeURIComponent(window.location.search).indexOf('Materiali') !== -1) {
            jQuery('.related-tabs li.tab-item[data-label-key="Materiali"]').trigger('click');
        }
	}
})