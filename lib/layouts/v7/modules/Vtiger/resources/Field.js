...
Vtiger_Field_Js('Vtiger_Picklist_Field_Js',{},{
	...
	isPicklistHeader : function(option) {
		return /^==\s*.*\s*==$/.test(option);
	},
	...
	getUi : function() {
		...
		for(var option in pickListValues) {
			var isPicklistHeader = this.isPicklistHeader(option);
			var optionClasses = [];
			if (isPicklistHeader) {
				optionClasses.push('picklistHeaderOption');
			}
			...
			if (optionClasses.length) {
				html += 'class="'+optionClasses.join(' ')+'" ';
			}
			if (isPicklistHeader) {
				html += 'disabled="disabled" data-picklist-header="true" ';
			}

			if(option == selectedOption && !isPicklistHeader) {
				html += ' selected ';
			}
