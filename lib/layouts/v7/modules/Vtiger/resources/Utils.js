...
var vtUtils = {
	...
	isPicklistHeaderLabel : function(label) {
		return /^==\s*.*\s*==$/.test(jQuery.trim(label || ''));
	},

	getPicklistHeaderLabel : function(label) {
		return jQuery.trim((label || '').replace(/^==\s*/, '').replace(/\s*==$/, ''));
	},

	registerPicklistHeaderStyles : function() {
		if (jQuery('#picklistHeaderOptionStyles').length) {
			return;
		}
		jQuery('head').append(
			'<style id="picklistHeaderOptionStyles" type="text/css">' +
				'.picklistHeaderOption, .picklistHeaderOption .select2-result-label { font-weight: 700 !important; }' +
				'.select2-result.picklistHeaderOption .select2-result-label { font-weight: 700 !important; }' +
				'.select2-result-unselectable.picklistHeaderOption, .select2-disabled.picklistHeaderOption { opacity: 1 !important; cursor: default !important; }' +
			'</style>'
		);
	},

	applyPicklistHeaderState : function(selectElement) {
		var element = jQuery(selectElement);
		if (element.attr('data-skip-picklist-header-block')) {
			return;
		}
		...
		instance.results
			.find('.select2-result.picklistHeaderOption')
			.removeClass('select2-result-selectable select2-highlighted')
			.addClass('select2-disabled select2-result-unselectable')
			.off('.picklistHeaderBlock')
			.on('mouseup.picklistHeaderBlock click.picklistHeaderBlock mousedown.picklistHeaderBlock', function(e) {
				e.preventDefault();
	...
	preparePicklistHeaders : function(selectElement) {
		var skipBlock = jQuery(selectElement).attr('data-skip-picklist-header-block');
		jQuery(selectElement).find('option').each(function() {
			var option = jQuery(this);
			if (vtUtils.isPicklistHeaderLabel(option.text())) {
				option.text(vtUtils.getPicklistHeaderLabel(option.text()));
				option.addClass('picklistHeaderOption');
				option.attr('data-picklist-header', 'true');
				if (!skipBlock) {
	...
	showSelect2ElementView : function(selectElement, params) {
		...
		vtUtils.registerPicklistHeaderStyles();
		vtUtils.preparePicklistHeaders(selectElement);
    ...
    if(jQuery('#minilistWizardContainer').length){params.maximumSelectionSize=4};
        ...
        selectElement.select2(params)
					 .on("open", function(e) {
						 ...
						 vtUtils.applyPicklistHeaderState(element);
						 if (instance.search && !instance.search.data('picklistHeaderBound')) {
						 	instance.search.on('keyup.picklistHeader input.picklistHeader', function() {
						 		setTimeout(function() {
						 			vtUtils.applyPicklistHeaderState(element);
						 		}, 0);
						 	});
						 	instance.search.data('picklistHeaderBound', true);
						 }
